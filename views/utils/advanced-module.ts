...
function getDataByType(type: DataType): DataRow[] {
  return get(pluginDataSelector(window.getStore()), [type, 'data'], [])
}
...
export function saveLog (
  ...
  const nickNameId: string = window.getStore('info.basic.api_nickname_id')
  if (nickNameId && parseInt(nickNameId) !== 0) {
    ...
    showMessage(t('User not found. Please ensure you\'re logged in'))
  }
}
...
export function importLog(
  ...
  if (nickNameId == null || parseInt(nickNameId) === 0) {
    showMessage(t("Please log in game before import data"))
    return
  }
  ...
  console.warn(`Open ${filename}`)
  if (filename != null && filename[0] != null) {
    try {
      fs.accessSync(filename[0], fs.constants.R_OK)
      ...
      fs.writeFile(path.join(DATA_PATH, 'akashic-records', "tmp", "data"), saveData.join('\n'))
      fs.emptyDirSync(path.join(DATA_PATH, 'akashic-records', nickNameId.toString(), logType.toLowerCase()))
      fs.writeFile(path.join(DATA_PATH, 'akashic-records', nickNameId.toString(), logType.toLowerCase(), "data"), saveData.join('\n'))
